...
module.exports = {
	makeList(req, res) {
		makeHDFSList(function (list){
			console.log(list)
		})

		//Load app list
		fs.readdir('./app', function (err, files){
			res.send({applist: files, datalist : makeHDFSList()});
		});
	},
	dataUpload(req, res){
		var DummyPath = 'app/'
		...
		form.parse(req, function(error, fields, files){
			...
			exec('hdfs dfs -put '+DummyPath+originalName + ' /' + dataFolder , function(err, stdout, stderr){
				...
				exec('rm '+DummyPath+originalName , function(err, stdout, stderr){
					...
					res.send({datalist : makeHDFSList(dataFolder)});
				});
			});
        	});
